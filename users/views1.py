...
def user_list(request, page = 1, msg = ''):
    ...
    return list_detail.object_list(
        request,
        queryset = UserProfile.objects.filter(query, user__is_staff
            = False).exclude(type='CC').order_by('-code'),
...
def manage_user(request, pid = None, RedirectView = user_list):
    ...
    if request.method == 'POST':
        if request.FILES:
            ...
            file_data = None
            profile_form = UserProfileForm(request.POST, file_data, instance = profile)
        ...
        if profile_form.is_valid() and  address_contact_form.is_valid() \
            ...
            for each in user.get_profile().business_unit.all():
                businessunit = each                      
            ...
            content ='Hi All,<BR><BR> This is to introduce ' + user.get_profile().salutation + '.' + user.first_name + 
            ', who has recently joined our ' + group + ". " + gender1 +
            " email id is " + user.email +              
            ".<BR><BR>Regards,<BR>Admin Group.<BR>Let us welcome "+ gender2 + " and make " + gender2 + " feel at home...",                        
            requested_by_id = '1',
            approved_by_id= '31'
            )
            announcement.save()         
            email_subject = 'New Announcement for approval'            
            email_message = 'A new announcement ' +  str(announcement.title) + ' has been created by System User'        
            Email().send_email(email_subject, email_message, ['sseshan@fifthgentech.com'],'html')                
    ...
    if request.method == 'POST':
        ...
        try:
            for profile_id in profiles:
                ...
                user.save()
            messages.success(request, _('User(s) status changed successfully.'))
        except Exception:
