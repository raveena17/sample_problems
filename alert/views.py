...
def alert_generate(request, module_name, alert_id, record_id, to_id = [], cc_id = [], comments=''):
    ...
    try: 
        ...
        if alert_id =='alertdataconfig18' or alert_id == 'alertdataconfig19' or alert_id == 'alertdataconfig20' or alert_id == 'alertdataconfig26' or alert_id == 'alertdataconfig27' :
            expend_tbl = Expenditure_Reimburs.objects.filter(reimbursement__id = record_id)
            ...
            body_content +='Total Amount:'+str(reimbus.total_expenditure)+'<br><br>'
...
def pay_it_status_hours_genrte(request):
    ...
    if request.method == 'POST':       
        ...
        ws.write_merge(0, 1, 0, len(header)-1, "Project-Wise Person Report", TITLE)
        ...
        SUBHEADER = easyxf('font: color black, bold on,height 220 ; align: '\
...
def _pay_it_status_convrsn(start_date, end_date, requested_by):
    ...
    _set_header(ws, "PM Report Sheet", date_header, requested_by, start_date, end_date)
    HEADER = easyxf('font: color black, bold on,height 220 ; align: '\
           'horiz left;pattern: pattern solid, fore_colour gray25, back_colour gray25')
    BODY = easyxf('font: color black,height 210;')
    for i, row in enumerate(list_data):
...
def task_report(request):
    ...
    if request.method == 'POST':
        ...
        date_s = datetime.strftime(fromDate, '%Y-%m-%d')
        date_e = datetime.strftime(toDate, '%Y-%m-%d')       
        timesheet_list = TaskTracking.objects.filter(start_time__range=[date_s,date_e])        
        task_list = dict(Task.objects.filter(id__in =\
            [a[0] for a in timesheet_list.values_list('task')]).values_list('id','name'))
        #task_dict = task_name
        project_list=dict(Project.objects.filter(id__in = [a[0] for a in timesheet_list.values_list('program')]).values_list('id','name'))
        header_list=['UserName','ProjectName','Tasks','StartTime','TimeSpent']
        ...
        for i, task in enumerate(timesheet_list):
            ...
            if task.program != None:
                project_name = project_list[long(float(task.program))]
            else:
                project_name="Non-Project Task"    
            result_list = [task.user.username,project_name, task_name, \
                                datetime.strftime(task.start_time, '%Y-%m-%d %H:%M:%S'), task.time_spent]
...
def _append_detail(list_data, users, date_s, date_e, list_date):
    ...
    for user in users:
        ...
        data = [user_code, user.first_name, '']
        time_spent = []
        tasks = TaskTracking.objects.filter(Q(user=user.id) & Q(start_time__range=[date_s, date_e+timedelta(1)]))
        projects = _get_projects(tasks)
        hours_count = 0
        missed_entry_count = 0
        leave_count = 0
        for date in list_date:
            time = tasks.filter(start_time__startswith=date).aggregate(Sum('time_spent')).values()
            if time[0] != None:
...
def sheet_report(request):
    ...
    if request.method == 'POST':
        ...
        extend_header = ['Total Spent Hrs.', 'No.Of Leaves', 'Not Entered/Days']
        ...
        filename = _save("Time Sheet Report", sheet_header, list_data, request.user.first_name, datetime.strftime(date_s, '%d-%m-%Y'), datetime.strftime(date_e, '%d-%m-%Y'))
        FILE_attach = settings.PAY_IT_STATUS_PATH
        filepath = FILE_attach
        wrapper = FileWrapper(file(filepath))
        response = HttpResponse(
...
def daily_report(request):
    ...
    if request.method == 'POST':
        ...
        filename = _save("Daily Report", sheet_header, list_data, request.user.first_name, datetime.strftime(date_s, '%d-%m-%Y'), datetime.strftime(date_e, '%d-%m-%Y'))
        FILE_attach = settings.PAY_IT_STATUS_PATH
        filepath = FILE_attach
        wrapper = FileWrapper(file(filepath))
        response = HttpResponse(
...
def _get_resources(task, date_s, date_e):
    return TaskTracking.objects.filter(start_time__range=[date_s,date_e+timedelta(1)], task=task)\
        .distinct().values_list('user_id', flat=True)

def _get_pjt_header(pjt_header, pjt, list_date):
    total_hrs = 0.0
    for date in list_date:
        hrs=TaskTracking.objects.filter(program=pjt, start_time__startswith=date).aggregate(Sum('time_spent')).values()
        if hrs[0] == None:
...
def _get_pjt_data(list_data, pjt, list_date, date_s, date_e):
    total_hrs = 0.0
    task_obj = TaskTracking.objects.filter(program=pjt, start_time__range=[date_s,date_e+timedelta(1)])
    ...
    for task in tasks:
        ...
        for resource in resources:
	        ...
	        tsk_hrs = daily.filter(task=task).aggregate(Sum('time_spent')).values()
                if tsk_hrs[0] != None:
...
def _project_detail(list_data, date_s, date_e, list_date):
    pjtid_list = TaskTracking.objects.filter(start_time__range=[date_s,date_e+timedelta(1)])
...
def project_report(request):
    ...
    if request.method == 'POST':
        ...
        filename = _save('Project Report', sheet_header, list_data, request.user.first_name, datetime.strftime(date_s, '%d-%m-%Y'), datetime.strftime(date_e, '%d-%m-%Y'))
        FILE_attach = settings.PAY_IT_STATUS_PATH
        filepath = FILE_attach
        wrapper = FileWrapper(file(filepath))
        response = HttpResponse(
