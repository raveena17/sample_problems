...
def manage_customer_from_project(request, id=None):
    ...
    return render(request, 'customer_from_project.html',
...
def manage_customer_contact_from_project(request, id=None):
    ...
    if request.method == 'POST':
        ...
        contact_profile_form = CustomerContactProfileForm(request.POST,
        ...
        if contact_form.is_valid() and address_contact_form.is_valid() \
            ...
            contact_profile = contact_profile_form.save(contact = contact,
        ...
        contact_profile_form = CustomerContactProfileForm(instance
                                        = contact_profile)
...
def clientlist(request, page = 1):
    ...
    callable = SubListView.as_view(
        queryset = BusinessUnit.objects.filter(query, type__name = 'Customer').order_by('is_active',
            '-customer_code'),
...
#    print request  
    return render(request, 'client_create.html', {'form': form,
        'address_form': address_form, 'project':project, 'task_id': task_id},
        )
